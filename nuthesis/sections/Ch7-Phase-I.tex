...
As discussed in chapter \ref{ch:lhcandcms} the CMS pixel detector will {\rojo{suffer}} from radiation damage throughout its lifetime hence the need for periodical upgrades. The first version of the detector was known as phase 0, it became fully operational 2010 after solving a setback during the original starting period in 2008. In 2017 the pixel detector was replaced during the so-called phase 1 upgrade, the University of Nebraska, high energy group (UNL-HEP) played a major role in assembling and testing over 500 modules, from 2013 to 2016, which then became part of the forward region of the pixel detector. The next update of this detector, the phase II upgrade, is projected to take place in 2025 when the current detector will be reaching its limits and its performance will be deteriorated. In this chapter we describe why the phase 0 pixel detector needed an upgrade making and {\rojo{describe}} the work done by the UNL-HEP group. Some of these steps will be highlighted and {\rojo{described}} in detail as they were my contributions to this production campaign. %Specially the  and highlighting 
...
The phase 0 detector performed well during the LHC run I, taking data at an initial luminosity of $7$ x $10^{33} cm^{-2} s^{-1}$, which was then used in many analysis including the discovery of the Higgs bosson published in 2013. But after a few years of operation the pixel detector started to degrade due to radiation damage, causing an increase of fake rates as well as loose on resolution. Moreover, for run II the LHC planned to double its luminosity with successive increment until it reaches its peak of $2$ x $10^{35} cm^{-2} s^{-1}$. A simulation of the performance of the pixel detector under different luminosity conditions can be seen in figure \ref{fig:red_perf}.
...
The current forward pixel detector have three endcap disks located at 29.1 cm, 39.6 cm, and 51.6 cm from the intraction point. Compared to the old FPix, innermost disk is 4.4 cm closer to the interaction point. This provides a four-hit coverage for tracks ranging up to $\pm$ 2.5 in pseudorapidity, as showing in figure \ref{schematicofpixeldetector}.
...
To reduce the extra material added by the extra layer of the pixel detector a lightweight support was used, a CO2 cooling system was adopted, and inactive materials were moved from the tracking area. % To avoid the increase of material that an additional layer signifies, several step by the addition of the extra layer
...
{\rojo{Overall the geometry of the current FPix is similar to the old detector.}} The three disks are vertically divided into half-disks, and are composed of inner and outer disks. This allows for easy assembly and disassembly for repairs when necessary. Each of these three disks are mounted on half cylinders, which also contains the CO2 cooling tubes and the readout electronics as shown in figure \ref{halfdisk}. Each half disk contains 56 modules bringing the total number of modules for the FPix to 672. One key difference between the old and current detector is the use of one single module type in a 2x8 array. This made easier the installation compared to the old detector.   
...
Modules are mounted on the inner and outer half disks and not attached to the half cylinder. This allows damage modules to be replaced without having to disassembly the half cylinder. The modules in the inner ring are rotated by 12 degrees towards the interaction point to improve resolution in both the azimuthal and radial direction.   

The size of the pixel is 100 x 150 mm. 
%\begin{figure}[!h]
%	\centering
